...
Switching projects with projectile updates treemacs to display the new project
exclusively, keeping the file tree in sync.
...
Only the first Emacs instance starts the server; later instances would
otherwise warn "Unable to start the Emacs server" on every startup
because the socket is already owned by the running server.
...
*** LLM inline completion in shell-mode

Inline completion works in shell buffers with two safety guards:
...
context so the model can suggest commands informed by error messages,
directory listings, and other output.

The guard plugs into the completion engine's block predicates (registered
in the minuet section below) so no automatic suggestion fires outside an
idle prompt.  Manual requests still work anywhere in a shell buffer.

A shell completion is also limited to a single line.  A FIM model sees
the session transcript as its context, so left unchecked it happily
continues the "document" with invented prompts and command output.  A
request transform (registered in the minuet section below) adds a
newline stop sequence when the completion originates from a shell
buffer, so generation ends with the current command line.
...
Emacs 29+ ships tree-sitter support but not the per-language grammar
shared libraries. Debian's =libtree-sitter0= package only provides the
core runtime, so modes like =yaml-ts-mode= fail to activate with a
"language grammar ... unavailable" warning until the matching
=libtree-sitter-LANG.so= exists under =tree-sitter/= in the user emacs
directory.

=treesit-install-language-grammar= clones a grammar repository and
compiles it with the system =cc=. Pre-populating
=treesit-language-source-alist= lets that command (and the block
below) find the source URL without prompting. The loop installs any
missing grammars on startup; failures are swallowed so that a transient
network or toolchain problem does not break =init.el=. Requires =git=
and a C compiler on =PATH=.

Grammar repositories that regenerate their parser with tree-sitter CLI
0.25+ produce ABI version 15, which Emacs 30's runtime (ABI <= 14)
refuses to load with a "version-mismatch" warning. Since a mismatched
grammar counts as unavailable, the loop would then rebuild the same
broken grammar on every startup. Such grammars are pinned to their
last ABI-14 release tag.
...
(defvar my/ollama-chat-model "qwen3.8-96k")          ; chat, rewrite, summarize
(defvar my/ollama-completion-model "qwen2.5-coder-fim") ; inline autocomplete
...
;; Use Claude API directly, Ollama for inline completion
...
[minuet](https://github.com/milanglacier/minuet-ai.el) provides inline
"as-you-type" ghost-text completions. It is configured with the
=openai-fim-compatible= provider, which sends a true fill-in-the-middle
request (=prompt= + =suffix=) to the =/v1/completions= endpoint. Ollama
handles that natively for FIM templates, so the local model can use its
native FIM tokens instead of an instruction-based marker protocol: base
models do not reliably follow marker tokens (they echo them back into
the completion), while a FIM template always does.

The completion model is =my/ollama-completion-model= (set in
=local-pre-init.el=); keep it a FIM-capable model such as
=qwen2.5-coder-fim=.

Toggle automatic suggestions with =C-q a TAB=; =C-q a p= requests a
suggestion immediately without waiting for the idle delay. While a
suggestion is shown, TAB accepts it, =M-a= accepts one line (with a
prefix, more lines), =M-w= accepts one word, =M-n=/=M-p= cycle
alternatives and =M-e= dismisses.
...
The =claude-code-ide-extras= package extends the base integration with
additional MCP tools grouped into four modules: Emacs introspection
(symbol docs, buffer search, elisp eval), LSP (format buffer, hover
info), Projectile (non-blocking task execution, project file/buffer
discovery), and meta (per-project tool advice via
=get_mcp_custom_advice=).

The Projectile module starts tasks in a per-project compilation buffer.
Setting =projectile-per-project-compilation-buffer= ensures parallel
tasks across projects do not clobber each other's output.
...
ECA (Editor Code Assistant) is an open-source AI pair programming tool. It
launches a local server process (downloaded automatically on first use) and
opens an interactive chat window.

Chat/rewrite use Anthropic (=my/claude-model=), while inline completion is
routed to the local Ollama completion model (=my/ollama-completion-model=) so the
fast, low-latency path stays on the LAN. Both are stitched together via
=ECA_CONFIG=, which ECA's server reads on startup without touching
=~/.config/eca/config.json=. Model selection uses the =provider/model= format
ECA expects (e.g. =anthropic/claude-opus-4-7=, =ollama/qwen2.5-coder-fim=).
...
Agent Shell runs AI coding agents (Claude Code, Gemini CLI, Codex, Goose) inside
a native Emacs comint buffer via the ACP (AI Control Protocol) standard. The
=agent-shell= command prompts for which agent to launch.
...
[[https://github.com/rhblind/emacs-mcp-server][emacs-mcp-server]] exposes this Emacs instance as an MCP server over a Unix
socket. It ships two tools: =eval-elisp= (arbitrary Elisp evaluation) and
=get-diagnostics= (Flycheck/Flymake errors across project buffers). Start it
manually with =C-q a M=; the socket lives at
=~/.emacs.d/emacs-mcp-server.sock=.

Security note: =eval-elisp= runs arbitrary Elisp, so keep the server off unless
an AI session actively needs it.

To let =agent-shell='s Claude Code session reach the server, register it once
with the Claude CLI (writes to =~/.claude.json=):

#+begin_example
claude mcp add emacs --scope user -- \
  python3 ~/.emacs.d/straight/repos/emacs-mcp-server/mcp-wrapper.py \
  ~/.emacs.d/emacs-mcp-server.sock
#+end_example

The bundled =mcp-wrapper.py= is pure-stdlib Python and bridges stdio to the
Unix socket, avoiding a =socat= dependency.
...
AI Code Interface provides a unified workflow across AI coding CLIs and an
Emacs MCP server. When a Claude Code session starts via =ai-code-cli-start=,
the package automatically launches a local MCP HTTP server and injects
session-scoped credentials into the agent, giving Claude Code access to
Emacs tools: buffer contents, project files, Flycheck/Flymake diagnostics,
imenu symbols, xref references, and tree-sitter AST inspection.

Editor-session tools (=editor_state=, =visible_buffers=, =messages_tail=,
=eval_elisp=) are enabled via =ai-code-mcp-editor-tools-enabled=.
