...
It uses the Ollama utility model (=my/ollama-utility-model=) and points at the
same Ollama host as the rest of the config. The Ollama URL is injected via
=ECA_CONFIG= so the ECA server inherits it without touching =~/.config/eca/config.json=.
Model selection uses the =provider/model= format ECA expects (e.g. =ollama/qwen2.5-coder:14b=).
