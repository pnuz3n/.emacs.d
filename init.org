...
Toggle automatic suggestions with =C-q a TAB=; while a suggestion is
shown, TAB accepts it, =M-a= accepts one line (with a prefix, more lines),
=M-w= accepts one word, =M-n=/=M-p= cycle alternatives and =M-e=
dismisses.
